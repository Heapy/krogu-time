...
class JapaneseChronologyJavaConformanceTest {
    ...
    fun chronologyDatesTransitionsFieldsAndArithmeticMatchJavaTime() {
        ChronoField.entries.filter(ChronoField::isDateBased).forEach { field ->
            ...
            assertEquals(
                ...
                groguOutcome { JapaneseChronology.range(field).toString() },
                field.name,
            )
        }
        ...
        JavaJapaneseEra.values().zip(JapaneseEra.values()).forEach { (javaEra, era) ->
            listOf(1, 2, 10).forEach { yearOfEra ->
                assertEquals(
                    ...
                    groguOutcome { JapaneseChronology.prolepticYear(era, yearOfEra).toString() },
                    "$era $yearOfEra",
                )
            }
        }
    ...
    private fun groguOutcome(block: () -> String): String =
        runCatching(block).fold(
            ...
            onFailure = {
                when (it) {
                    is io.heapy.grogu.time.temporal.UnsupportedTemporalTypeException ->
                        "java.time.temporal.UnsupportedTemporalTypeException"
                    is io.heapy.grogu.time.DateTimeException -> "java.time.DateTimeException"
